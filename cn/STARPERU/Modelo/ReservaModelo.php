...
//        }
        ...
        $consulta = "(SELECT r.Registro,
                        ...
                        WHERE NOT EXISTS (
                        SELECT v2.fechahora_transaccion,r.Registro
                        FROM visa AS v2
                        WHERE  v2.reserva_id= r.Registro AND  v.fechahora_transaccion < v2.fechahora_transaccion) AND e.CodigoEntidad = $codigo_entidad $filtro
                        )
			...
			SELECT  v2.fechahora_transaccion,r.Registro
			FROM visa AS v2
			WHERE  v2.reserva_id= r.Registro AND v.fechahora_transaccion < v2.fechahora_transaccion) AND e.CodigoEntidad = $codigo_entidad $filtro AND  r.TipoVuelo = 'R' 
                        )
                        ORDER BY FechaRegistro DESC ,tramo ASC $limite";
...
//     return $res;
// }
    public function GuardarReservaCabecera($codigo_reserva, $nombres, $apellidos, $email, $tipo_documento, $numero_documento, $telefono, $anexo, $celular, $nextel, $rpm, $rpc, $pasajero_ruc, $fecha_registro, $fecha_limite, $adultos_5, $menores_5, $infantes_5, $origen_ida_5, $destino_ida_5, $numero_vuelo_ida_5, $clase_ida_5, $fecha_salida_ida_5, $hora_salida_ida_5, $numero_vuelo_vuelta_5, $clase_vuelta_5, $fecha_salida_vuelta_5, $hora_salida_vuelta_5, $pais, $ciudad, $ip, $flete, $tuua_5, $igv_5, $total_pagar_5, $usuario, $entidad, $tipo_vuelo_letras) {
        $flag = 0;
    ...
    public function GuardarReservaDetalle($registro, $j, $tipo_documento, $numero_documento, $apellidos, $nombres, $tipo_pax, $celular, $telefono, $anexo, $rpc, $rpm, $email, $tarifa_unitaria, $igv_unitaria, $tuua_unitaria, $total, $total_pagar, $fecNac) {
        $flag = 0;
        $obj_conexion = new ConexionBD();
        $conexion = $obj_conexion->CrearConexion();
