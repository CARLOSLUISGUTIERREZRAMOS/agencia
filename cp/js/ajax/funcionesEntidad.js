...
window.onload=function(){ $('#ruc_e').focus(); };
...
function registraEntidad()
{
	...
	if( sw==1 && array_input_vacio.length==0 )
	{
		...
		}else{ div_aviso_error('#div-entidad-aviso', 'Error, los datos de RUC o DNI no cumplen con el Formato.'); }
	}else{
...
function cambiaEstado(var_id,var_estado)
{
	...
	{
		consulta=confirm("Se activará la Entidad Seleccionada?");
	}
	if(consulta)
	{
...
function busqueda(pag)
{
	...
	if( val_url.length>0 )
	{
		...
		if(objAjax.texto.length>0)
		{
			...
			$('#recargar-lista').html(linkRecarga);
			$('#listaResultado').html('<p>No existen Datos para Visualizar. Por favor seleccione otras opciones de búsqueda.</p>').css({'font-weight': 'bold', 'font-size': '15px', 'color':'#000099'});
		}
	}
}
/* ======================================  SALDO ==============================================*/
